...
uintptr_t FindPattern(uintptr_t begin, size_t size, const char *pattern)
{
    ...
    // Probe the first page of the range once rather than VirtualQuery
    // each byte. .text is typically one contiguous committed region,
    // so this is enough for the scan to run cleanly. A caller passing
    // garbage ranges will get 0 back via the probe failure.
