...
// any byte.
//
//   FindPattern(text_begin, text_size, "8B 05 ?? ?? ?? ?? 85 C0");
//
// Returns the address of the first byte of the first match inside
// [begin, begin+size), or 0 if nothing matched. Scans byte-by-byte
// (unaligned).
uintptr_t FindPattern(uintptr_t begin, size_t size, const char *pattern);

// Count-aware variant. Scans the full range, writes up to
// `out_capacity` match addresses into `out_matches`, and returns
// the total number of matches found (which may exceed the
// capacity — the caller learns how ambiguous the signature is
// even when it only keeps the first few matches).
//
// Typical use:
//
//   uintptr_t m[4];
