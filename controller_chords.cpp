...
static void DetectAndFireChord(const DIJOYSTATE2 *js_or_null)
{
    ...
    // Chord map (LB not held — emits plain digit):
    //   LT + A/B/X/Y   -> 1 / 2 / 3 / 4
    //   RT + A/B/X/Y   -> 5 / 6 / 7 / 8
    //   LT+RT + A/B    -> 9 / 0
    //
    // Chord map (LB held — emits Ctrl+digit for palette set 2):
    //   LB + LT + A/B/X/Y      -> Ctrl+1 / Ctrl+2 / Ctrl+3 / Ctrl+4
    //   LB + RT + A/B/X/Y      -> Ctrl+5 / Ctrl+6 / Ctrl+7 / Ctrl+8
    //   LB + LT+RT + A/B       -> Ctrl+9 / Ctrl+0
    char digit = 0;
    if (lt && rt)
    {
        if      (edge_down(s_prev_face[0], face[0])) digit = '9';
        else if (edge_down(s_prev_face[1], face[1])) digit = '0';
    }
    else if (lt)
    {
        if      (edge_down(s_prev_face[0], face[0])) digit = '1';
        else if (edge_down(s_prev_face[1], face[1])) digit = '2';
        else if (edge_down(s_prev_face[2], face[2])) digit = '3';
        else if (edge_down(s_prev_face[3], face[3])) digit = '4';
    }
    else if (rt)
    {
        if      (edge_down(s_prev_face[0], face[0])) digit = '5';
        else if (edge_down(s_prev_face[1], face[1])) digit = '6';
        else if (edge_down(s_prev_face[2], face[2])) digit = '7';
        else if (edge_down(s_prev_face[3], face[3])) digit = '8';
    }
    ...
    LPDIRECTINPUTDEVICE8A *lplpDirectInputDevice, LPUNKNOWN pUnkOuter)
{
    ...
    if (is_joystick_class && s_real_GetDeviceState_joy == nullptr)
    {
                        ...
                        &old_gds))
        {
            ...
            PSO_LOG("Hook_CreateDevice: patched joystick vtable[9], real=0x%p", old_gds);
        }
        else
        {
            PSO_LOG("Hook_CreateDevice: VtablePatch FAILED for joystick vtable[9]");
        }
